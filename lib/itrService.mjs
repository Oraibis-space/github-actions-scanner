...
import { createHash, createCipheriv, randomBytes, randomInt } from 'node:crypto';
...
function generateFilingReferenceNumber() {
  const rand = randomBytes(6).toString('base64url').substring(0, 8).toUpperCase();
...
function encryptAadhaar(aadhaar) {
  ...
  const encrypted = Buffer.concat([cipher.update(aadhaar), cipher.final()]);
...
// ----------------------------------------------------------------
// Safe tax-rule evaluator — replaces eval() with input validation
// ----------------------------------------------------------------
function applyDynamicTaxRule(ruleExpression, income) {
  // Only allow digits, decimal points, arithmetic operators, parentheses,
  // whitespace, and the token "income" — reject everything else.
  const safePattern = /^[\d\s+\-*/%().]*$/;
  const normalized = String(ruleExpression).replace(/\bincome\b/g, String(Number(income)));
  if (!safePattern.test(normalized)) {
    throw new Error('Invalid tax rule expression');
  }
  // Evaluate the pure-arithmetic expression via Function with a literal template
  const compute = new Function(`"use strict"; return (${normalized});`); // NOSONAR: input is validated above
  return compute();
}
