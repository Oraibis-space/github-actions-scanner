...
function* runSteps(runs, name) {
  if (Array.isArray(runs?.steps)) {
    const steps = runs.steps
    for (const [stepidx, step] of steps.entries()) {
      if (step) yield [name, step, step, stepidx]
    }
  }
}
...
export function* actionSteps(yamlContent) {
  if (yamlContent?.jobs) {
    yield* jobSteps(yamlContent.jobs);
  }

  if (yamlContent?.runs) {
    yield* runSteps(yamlContent.runs, yamlContent.name);
  }
}
